...
read-only токену достаточно. Ходим через официальный SDK `t-tech-investments` (он не на
PyPI — индекс прописан в `[tool.uv.sources]`). Возможные исходы:

| Что ответил брокер | Ответ API | Что в БД |
|---|---|---|
| успех | `200 {"has_token": true}` | токен записан |
| `UNAUTHENTICATED`, `PERMISSION_DENIED` | `400`, code `invalid_token` | без изменений |
| любой другой статус, таймаут, сбой TLS/сети | `503`, code `upstream_unavailable` | без изменений |

До брокера токен ещё проходит проверку схемой: он обрезается по краям и обязан состоять
из печатаемых ASCII-символов, иначе `422` code `validation_error`. Это не косметика —
и gRPC-метадата, и HTTP-заголовки ASCII-only, а токен приезжает вставленным из чата, так
что кириллическая «т» или невидимый символ внутри роняли запрос ошибкой кодирования в
транспорте, и пользователь видел `500`. Формат самого токена не проверяется — это вердикт
брокера.
...
через `TINVEST_GRPC_TARGET` и `TINVEST_TIMEOUT_SECONDS`.

**`SSL_TBANK_VERIFY=true` обязателен.** T-Invest отдаётся под «Русским доверенным
корневым УЦ», которого нет ни в одном дефолтном хранилище: без этой переменной SDK берёт
системные корни, TLS-рукопожатие падает, и каждая проверка токена превращается в `503`.
С ней используется сертификат, который SDK везёт с собой. В образе она проставлена в
`Dockerfile`, локально — в `.env`.
